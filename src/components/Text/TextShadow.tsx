...
import { IText } from "../../interface.tsx";
...
const TextShadow: React.FC = () => {
  ...
  const [layers, setLayers] = useState<IText[]>([]);
  ...
  }, [shiftRight, shiftDown, blur, opacity]);
  ...
  const updateTextShadowValue = () => {
    const textShadow = `${shiftRight}px ${shiftDown}px ${blur}px rgba(0, 0, 0, ${
      opacity / 100
    })`;
  ...
  const handleChangeLayer = (index: number): void => {
    ...
    setChoiceColor(choiceLayer.rgbaColor);
  };
    ...
    <>
      ...
      <main className="flex min-h-screen flex-col items-center justify-between pt-[20px]">
        <div className="container mx-auto px-[100px] grid grid-cols-2 gap-5">
          ...
          <div>
            <Preview textShadowValue={textShadowValue} />
            <CssCode textShadowValue={textShadowValue} />
          </div>
        </div>
      </main>
    </>
